                    ...
                    if(t < token_cnt)
                    {
                        for(int k = 0; k < K; ++k)
                        {
                            ...
                            if constexpr(is_same_v<BDataType, f4x2_pk_t>)
                            {
                                ...
                                v_b = type_convert<ComputeTypeB>(f4) * type_convert<ComputeTypeB>(b_scale);
                                v_b_up = type_convert<ComputeTypeB>(f4_up) * type_convert<ComputeTypeB>(b_scale_up);
                            }
                            else
                            {
                                v_b = type_convert<ComputeTypeB>(b_f4x2) * type_convert<ComputeTypeB>(b_scale);
                                v_b_up = type_convert<ComputeTypeB>(b_f4x2_up) * type_convert<ComputeTypeB>(b_scale_up);
                                arg.b_element_op_(v_b, v_b);
                                arg.b_element_op_(v_b_up, v_b_up);
                            }

                            v_acc    += ck::type_convert<AccDataType>(v_a) * ck::type_convert<AccDataType>(v_b);
                            v_acc_up += ck::type_convert<AccDataType>(v_a) * ck::type_convert<AccDataType>(v_b_up);
                        }
                        ...
                        printf("%.1f, ", type_convert<float>(v_acc));
                        //printf("%.1f, ", type_convert<float>(v_acc_up));
                        //printf("[%.1f, %.1f], ", type_convert<float>(v_acc), type_convert<float>(v_acc_up));
                        ...
                        else if constexpr(ActivationType == 0)
                        {
                            tensor_operation::element_wise::Gelu{}(v_c, v_c);
                            //arg.c_t_k_n_(t, topk_id, n) = v_c * v_c_up;
                            //printf("[%f, %f], ", type_convert<float>(v_c), type_convert<float>(v_c_up));
                            //printf("[%d-%d] %f\n", t, topk_id, type_convert<float>(v_c * v_c_up));
                            //printf("[%d-%d] %f\n", t, topk_id, type_convert<float>(arg.c_t_k_n_(t, topk_id, n)));
                        }
                    }
                }
            }
