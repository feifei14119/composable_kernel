...
int main(int argc, char* argv[])
{
    ...
    printf("sorted_tile_num = %d,\tvalid_tile_num = %d\n", sorted_tile_num, valid_tile_num);
    printf("sorted_size     = %d,\tvalid_size     = %d\n", sorted_size, valid_size);
    printf("token_per_tile = %d\n", token_per_tile);
    printf("experts = %d, tokens = %d, topk = %d\n", experts, tokens, topk);
    expert_ids.savetxt("expert_ids.txt", "int");
    sorted_token_ids.savetxt("sorted_token_ids.txt", "int");

    Tensor<A0DataType> a0_t_k(HostTensorDescriptor({tokens, K}, {K, 1}));
    Tensor<XDataType> a1_t_k(HostTensorDescriptor({tokens, (K + ScaleBlockSize - 1) / ScaleBlockSize},
                                                  {Scale_Stride_AM, 1}));
    Tensor<B0DataType> b0_e_n_k(HostTensorDescriptor({experts, K, N * 2}, {N * 2 * K, 1, K}));
    Tensor<XDataType> b1_e_n_k(HostTensorDescriptor({experts, (K + ScaleBlockSize - 1) / ScaleBlockSize, N * 2},
                                                    {(N * 2 * Scale_Stride_BN), 1, Scale_Stride_BN}));

    // A, B Scale preshuffle
    Tensor<XDataType> a_scale_sorted(HostTensorDescriptor({sorted_size, (K + ScaleBlockSize - 1) / ScaleBlockSize}, 
                                                          {Scale_Stride_AM, 1}));
    Tensor<XDataType> a_scale_preshuffled(HostTensorDescriptor({sorted_size, (K + ScaleBlockSize - 1) / ScaleBlockSize}, 
                                                               {Scale_Stride_AM, 1}));
    Tensor<XDataType> b_scale_preshuffled(HostTensorDescriptor({experts, (K + ScaleBlockSize - 1) / ScaleBlockSize, N * 2},
                                                               {N * 2 * Scale_Stride_BN, 1, Scale_Stride_BN}));
    Tensor<D2DataType> d2_e_n(HostTensorDescriptor({sorted_size, N}, {1, 0}));
    Tensor<EDataType> e_t_k_n_host_result(HostTensorDescriptor({tokens, topk, N}, {topk * N, N, 1}));
    Tensor<EDataType> e_t_k_n_device_result(HostTensorDescriptor({tokens, topk, N}, {topk * N, N, 1}));
    ...
    // A/B scale shuffle
    preShuffleScaleBuffer<ck::is_same_v<A0Layout, Row>>(a_scale_sorted.mData.data(), a_scale_preshuffled.mData.data(), sorted_size, K / ScaleBlockSize);
    // feifei: 2 buff shuffle
    preShuffleScaleBuffer<ck::is_same_v<B0Layout, Col>>(b1_e_n_k.mData.data(), b_scale_preshuffled.mData.data(), N * 2 * experts, K / ScaleBlockSize);

    /*printf("scale a: \n");
    for(int st = 0; st < sorted_size; st++)
    {
        for(int k = 0; k < (K + ScaleBlockSize - 1) / ScaleBlockSize; k++)
        {
            int idx = st * ((K + ScaleBlockSize - 1) / ScaleBlockSize) + k;
            printf("%.1f, ", type_convert<float>(a_scale_sorted.mData[idx]));
        }
        printf("\n");
    }
    printf("preshuffled scale a: \n");
    for(int st = 0; st < sorted_size; st++)
    {
        for(int k = 0; k < (K + ScaleBlockSize - 1) / ScaleBlockSize; k++)
        {
            int idx = st * ((K + ScaleBlockSize - 1) / ScaleBlockSize) + k;
            printf("%.1f, ", type_convert<float>(a_scale_preshuffled.mData[idx]));
        }
        printf("\n");
    }*/
    /*printf("scale b: \n");
    for(int e = 0; e < experts; e++)
    {
        ...
        for(int n = 0; n < N; n++)
        {
            for(int k = 0; k < (K + ScaleBlockSize - 1) / ScaleBlockSize; k++)
            {
                int idx = e * (N * 2) * ((K + ScaleBlockSize - 1) / ScaleBlockSize)
                        + n * ((K + ScaleBlockSize - 1) / ScaleBlockSize)   
                        + k;
                printf("%.1f, ", type_convert<float>(b1_e_n_k.mData[idx]));
            }
            printf("\n");
        }
        printf("b2: \n");
        ...
        for(int n = 0; n < N; n++)
        {
            for(int k = 0; k < (K + ScaleBlockSize - 1) / ScaleBlockSize; k++)
            {
                int idx = e * (N * 2) * ((K + ScaleBlockSize - 1) / ScaleBlockSize)
                        + n * ((K + ScaleBlockSize - 1) / ScaleBlockSize)   
                        + k;
                printf("%.1f, ", type_convert<float>(b_scale_preshuffled.mData[idx]));
            }
            printf("\n");
        }
        printf("b2: \n");
    ...
    if(do_verification)
    {
        ...
        for(int m = 0; m < valid_size; ++m)
        {
            ...
            for(int n = 0; n < N; ++n)
            {
                e_t_k_n_host_result(t, topk_id, n) = ck::type_convert<EDataType>(c_t_k_n(t, topk_id, n));
            }
        }
